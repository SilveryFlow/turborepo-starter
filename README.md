...
>
> 切换分支命令：`git switch main` 或 `git switch bun`

基于 Bun + Turborepo 构建的 Vue 3 Monorepo 项目模板，用于开发可共享的 UI 组件库和应用程序。
...
| `bun run lint`         | Lint 所有包                    |
| `bun run format`       | 格式化所有代码（oxfmt）        |
| `bun run format:check` | 检查格式化                     |
| `bun run spell`        | 拼写检查                       |
| `bun run type-check`   | 类型检查                       |
| `bun run test`         | 运行所有测试                   |
| `bun run test:watch`   | 运行测试监视模式               |
| `bun run app:new`      | 创建新的 workspace（应用或包） |
| `bun run app:copy`     | 复制现有 workspace             |
| `bun run commit`       | 提交代码（使用 Commitizen）    |
...
# 仅类型检查 template-app
bun run --filter @repo/template-app type-check

# 仅 lint utils 包
bun run --filter @repo/utils lint
```
...
- **极快的安装速度** - 得益于全局模块缓存和高效的依赖解析
- **原生 Workspace 支持** - 内置 monorepo workspace 功能
- **一体化工具链** - 集成了运行时、包管理器、打包器和测试运行器
